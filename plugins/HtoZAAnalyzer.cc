...
void HtoZAAnalyzer::analyze(const edm::Event& event, const edm::EventSetup&, const ProducersManager& producers, const AnalyzersManager&, const CategoryManager&) {
    ...
    for (unsigned int ilep1 = 0; ilep1 < leptons.size(); ilep1++)
    {
        ...
        for (unsigned int ilep2 = ilep1+1; ilep2 < leptons.size(); ilep2++)
        {
            ...
            // Store NNLL reweighting for Z peak -->
            // https://github.com/latinos/PlotsConfigurations/blob/master/Configurations/ggH/Full2016/samples.py#L173
            float gen_ptll = dilep.gen_p4.Pt();
            if (gen_ptll < 140.) {
                dilep.ptllDYW_NNLL = (0.876979+gen_ptll*(0.00411598)-(0.0000235520)*gen_ptll*gen_ptll)*(1.10211 * (0.958512 - 0.131835*std::erf((gen_ptll-14.1972)/10.1525))); 
            }
            else {
                dilep.ptllDYW_NNLL = 0.891188;
            }
